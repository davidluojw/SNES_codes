...
int main(int argc, char **args) {
    ...
    PetscErrorCode ierr;
    PetscMPIInt    size;
    PetscReal      atol = 1.0e-8;  // 残差的绝对值阈值
    PetscReal      rtol = 1.0e-50;  // 残差的相对值（与初始残差相比）阈值
    PetscReal      stol = 1.0e-50;  // 解的范数残差
    PetscInt       maxit = 100;  // 最大迭代次数
    PetscInt       maxf = 100; // 最大函数评估次数
    ...
    // SNESQNSetType(snes, SNES_QN_LBFGS);                  // SNES_QN_BROYDEN, SNES_QN_BADBROYDEN, SNES_QN_LBFGS
    // SNESQNSetScaleType(snes, SNES_QN_SCALE_JACOBIAN);    // SNES_QN_SCALE_DIAGONAL, SNES_QN_SCALE_NONE, SNES_QN_SCALE_SCALAR, SNES_QN_SCALE_JACOBIAN
    // SNESQNSetRestartType(snes, SNES_QN_RESTART_PERIODIC);  // SNES_QN_RESTART_NONE, SNES_QN_RESTART_PERIODIC, SNES_QN_RESTART_POWELL
    ...
    /* 设置 Line Search 的类型和阈值 */
    PetscReal      ls_steptol = 0.3;   // The minimum steplength, default: 1e-12
    PetscReal      ls_maxtol = 16;     // The maximum steplength, default: 1e+08
    PetscReal      ls_rtol = 0.5;      // The relative tolerance for iterative line searches, default: 1e-08
    PetscReal      ls_atol = 1.0e-50;  //  The absolute tolerance for iterative line searches, default: 1e-15
    PetscReal      ls_ltol = 1.0e-50;  // The change in lambda tolerance for iterative line searches, deault: 1e-08
    PetscInt       ls_maxit = 10;         // The maximum number of iterations of the line search, deault:  1
    // SNESGetLineSearch(snes, &linesearch);
    // SNESLineSearchSetType(linesearch, SNESLINESEARCHCP);
    // SNESLineSearchGetTolerances(linesearch, &ls_steptol, &ls_maxtol, &ls_rtol, &ls_atol, &ls_ltol, &ls_maxit);
    // std::cout << "ls_steptol " << ls_steptol << std::endl;
    // std::cout << "ls_maxtol " << ls_maxtol << std::endl;
    // std::cout << "ls_rtol " << ls_rtol << std::endl;
    // std::cout << "ls_atol " << ls_atol << std::endl;
    // std::cout << "ls_ltol " << ls_ltol << std::endl;
    // std::cout << "ls_maxit " << ls_maxit << std::endl;
    // SNESLineSearchSetTolerances(linesearch, ls_steptol, ls_maxtol, ls_rtol, ls_atol, ls_ltol, ls_maxit);
